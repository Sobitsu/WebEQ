...
function TestSupport() {
    this.wadlDoc = null;
    this.wadlURL = '';
    this.wadlErr = 'Файл WADL недоступен. Перезапустите приложение RESTful и обновите эту страницу.';
    ...
    this.viewIds = [
        { "id" : "table" , "name":"Представление в табличной форме", "type":"tableContent"}, 
        { "id" : "raw" , "name":"Представление необработанных данных", "type":"rawContent"}, 
        { "id" : "structure" , "name":"Подресурс", "type":"structureInfo"},
        { "id" : "header" , "name":"Заголовки", "type":"headerInfo"},
        { "id" : "monitor" , "name":"Монитор HTTP", "type":"monitorContent"}];
...
TestSupport.prototype = {    
    ...
    initFromWadl : function () {
        ...
        if(wDoc != null) {
            ...
            this.updatepage('content', '<span class=bld>Страница справки</span><br/><br/><p>Файл WADL недоступен. Перезапустите приложение REST и обновите эту страницу.</p><p>Если это сообщение появляется вновь при просмотре страницы с помощью Firefox c подключаемым модулем Firebug,<br/>необходимо отключить Firebug для локальных файлов. В меню Firefox установите флажок <br/>"Инструменты" > "Firebug" > "Отключить Firebug для локальных файлов"</p>');
        }            
    },
    ...
    getMethodMimeTypeCombo : function (resource) {
        ...
        var str = '<table border=0><tbody><tr><td valign="top"><span id="j_id14"><label for="methodSel" class="LblLev2Txt_sun4">'+
                            '<span>Выберите метод для тестирования: </span></label></span></td>';
        ...
        str += "</select></span></td><td width=46/><td><a class='Btn1_sun4 Btn1Hov_sun4' onclick='ts.testResource()'>Тестировать</a></td></tr></tbody></table>";
        return str;
    },
    ...
    doShowDynamicResource : function (uri, mName, mediaType) {
        ...
        str += "</select></span></td>";
        str += "<td width=30/>"
        str += "<td><span id=j_id14><a class='Btn2_sun4 Btn1Hov_sun4' onclick='ts.addParam()'>Добавить параметр</a>";
        str += "</span></td><td><a class='Btn1_sun4 Btn1Hov_sun4' onclick='ts.testResource()'>Тестировать</a></td></tr></tbody></table><br/>";
        ...
        this.updatepage('request', '<span class=bld>Ресурсы:</span> '+uriLink+' <br/>(<a href="'+req+'" class=Hyp_sun4 target="_blank"><span>'+this.getDisplayURL(req, 90)+'</span></a>)');
    },
    ...
    clearAll : function() {
        ...
        this.updatepage('request', 'Для тестирования выберите узел на панели переходов (в левой части этой страницы).');
    ...
    getParamRep : function (req, mName) {
        ...
        if(mName == 'PUT' || mName == 'POST') {   
            str += '<tr><td valign="top"><span id="j_id14"><label for="blobParam" class="LblLev2Txt_sun4">'+
                '<span>Содержимое: </span></label></span></td>'+
                '<td><span id="j_id14"><textarea class="TxtAra_sun4 TxtAraVld_sun4" id=blobParam name=params rows=6 cols=65>Вставьте содержимое.</textarea></span></td></tr>';
        }
        str += '</tbody></table>';
        return str;
    },
    ...
    testResource : function () {
        this.updatepage('result', 'Загрузка...');
    ...
    createIFrameForUrl : function (url) {
        var c = 
            '<iframe id="iFrame_" src="'+url+'" class="frame" width="'+ts.iframeWidth+'" align="left">'+
                '<p>См. <a class=Hyp_sun4 href="'+url+'">"'+url+'"</a>.</p>'+
            '</iframe>';
        return c;
    },
    ...
    monitor : function (method, xmlHttpReq, param) {
        ...
        var s = 'Запрос: ' + this.currentMethod + ' ' + cURL + '\n<br/>' + params +
                    '\n\n<br/><br/>Статус: ' + xmlHttpReq.status + ' (' + xmlHttpReq.statusText + ')'+
                    '\n\n<br/><br/>Временная метка: ' + ' ' + xmlHttpReq.getResponseHeader('Date') + '';
        var prevs = '';
    ...
    updateContent : function (content) {
        var showRaw = true;
        var cErr = 'Для содержимого могут отсутствовать отношения "контейнер - содержимое". Содержимое можно просмотреть в представлении необработанных данных.';
    ...
    createTabs : function(tableContent, rawContent, showRaw) {
        ...
        if(subResources == null)
            subResources = 'Отсутствуют доступные подресурсы.';
        var tabs = '<br/><span class=bld>Статус:</span> '+ this.currentXmlHttpReq.status+' ('+this.currentXmlHttpReq.statusText+')<br/><br/>'+
                    '<span class=bld>Ответ:</span> '+
    ...
    getTab : function (id, actived) {
        ...
        if(actived)
            return '<td class="Tab1TblSelTd_sun4"><div title="Текущий выбор: текстовое поле" class="Tab1SelTxtNew_sun4"><a name="selectedTabAnchor" id="tab'+id+'"/>'+name+'</div></td>';
        else {
            if(id == '')
    ...
    debug : function(message) {
        ...
        if(dbgComp == null) {
            ...
            dbgComp.innerHTML = '<table><tr><td><span style="color: blue">Окно отладки REST</span></td><td>'+addActionStr + '</td></tr></table><br/>';
        }
    ...
    getHttpRequest : function() {
        ...
        {    // Internet Explorer 6.0+, 5.0+
            ...
            catch (e)
            {
                ...
                {
                    ts.debug("Этот браузер не поддерживает AJAX!");
                }
            }
        }
        return xmlHttpReq;
        ...
        try {
            ...
            if (this.isResponseReady(method, xmlHttpReq, content, monitor)) {
              ...
              if(rtext == undefined || rtext == '' || rtext.indexOf('HTTP Status') != -1) {
                  var err = method+' ЗапросFailed RequestFailed --> Статус: (' 
                  	   + xmlHttpReq.status+')\n<br/>'+
                      'Ответ: {<br/>' + xmlHttpReq.responseText + "<br/>}";
